...
const createRulesetFunctionMock = jest.fn((_config, fn: unknown) => fn);

let overrideSeverity: typeof import('../../functions/overrideSeverity.js').default;

beforeAll(async () => {
  jest.resetModules();
  await jest.unstable_mockModule('@stoplight/spectral-core', () => ({
    __esModule: true,
    createRulesetFunction: createRulesetFunctionMock,
  }));

  ({ default: overrideSeverity } = await import('../../functions/overrideSeverity.js'));
});

beforeEach(() => {
  createRulesetFunctionMock.mockClear();
});
